...
for _ in range(iterations):
  ...
  df_original=pd.read_csv('path_to_frenetic_testsuite.csv')
  df_pass = df_original.loc[df_original['algo_safety'] == 0]
  df_fail = df_original.loc[df_original['algo_safety'] == 1].sample(n=len(df_pass.index))
  ...
  random_features_dataframe = df.loc[:, df.columns.str.contains('feature')].sample(n=4, axis='columns')
  random_feature_labels = list(random_features_dataframe.columns.values)
  print('feature labels random: ' , random_feature_labels)
  random_algo_dataframe = df.loc[:, df.columns.str.contains('algo')] 
  ...
  ISA_features = ['feature_median_angle', 'feature_num_r_turns', 'feature_road_distance']
  ISA_features_dataframe = df[ISA_features]
  ...
  ISA_algo_dataframe = df.loc[:, df.columns.str.contains('algo')] 
  ...
  print("*************Using a separate dataset for testing*************")
  test_df_original = pd.read_csv('path_to_AmbieGen_testsuite.csv')

  test_df_pass = test_df_original.loc[test_df_original['algo_safety'] == 0]
  test_df_fail = test_df_original.loc[test_df_original['algo_safety'] == 1].sample(n=len(test_df_pass.index))
  # test_df = pd.concat([test_df_pass, test_df_fail])

  test_df = test_df_original


  random_test_features_dataframe = test_df[random_feature_labels]
  random_test_algo_dataframe = test_df.loc[:, test_df.columns.str.contains('safety')] 

  random_test_features=np.array(random_test_features_dataframe)
  random_test_labels=np.array(random_test_algo_dataframe)


  ISA_test_features_dataframe = test_df[ISA_feature_labels]
  ISA_test_algo_dataframe = test_df.loc[:, test_df.columns.str.contains('safety')] 

  ISA_test_features=np.array(ISA_test_features_dataframe)
  ISA_test_labels=np.array(ISA_test_algo_dataframe)
...
df_performance_comparison.to_csv('path_to_output/performance-results.csv')
